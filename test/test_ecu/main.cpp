...
void safety_assert_failed_handler(AssertLevel level, LineInfo info, AssertCode error_code) {
    ...
    if (expecting_safety_violation) {
        ...
        * know that we failed the test. */
        ...
        fail_msg << "Assert failed at " << info.filename << ":" << info.line_no << " with code "
...
void test_can_msg_eql(CAN_message_t msg, MessageId id, uint8_t* buf, size_t len) {
    ...
    int result = memcmp(&msg.buf, buf, len);
...
void wait_while_sending_pedal_positions(
    ...
    for (uint32_t i = 0; i < duration / 100; i++) {
        ecu->processMessage(*current_time_ms, create_throttle_one_position(throttle_1));
        ecu->processMessage(*current_time_ms, create_throttle_two_position(throttle_2));
        ecu->processMessage(*current_time_ms, create_brake_pressure(brake));
...
void test_ecu() {
    ...
    test_can_msg_eql(ecu.poll(current_time_ms).value(), MessageId::ControlCommand, enabled_but_no_torque, 8);
    TEST_ASSERT(ecu.poll(current_time_ms) == std::nullopt);

    /* Let's input throttle one. */
    ecu.processMessage(current_time_ms, create_throttle_one_position(THROTTLE1_HIGH));
    current_time_ms += 20;
    ...
    ecu.processMessage(current_time_ms, create_brake_pressure(BRAKE_CONSIDERED_PRESSED));
    ecu.processMessage(current_time_ms, create_throttle_two_position(THROTTLE2_HIGH));
    // Cycle the pedal value through history. We should hit it somewhere in here.
    int i = 0;
    while (!hit_safety_violation && i < 100) {
        current_time_ms += 20;
        ecu.poll(current_time_ms);
        i++;
    }
    if (i == 100) TEST_ASSERT(false); /* Never hit safety violation. */
    ...
    /* Cycle throttle history. */
    wait_while_sending_pedal_positions(&ecu, &current_time_ms, 1000, THROTTLE1_HIGH, THROTTLE2_HIGH, BRAKE_PRESSURE_MIN);
    ...
    uint8_t enabled_with_torque[] = {0x00, 0x00, 0x00, 0x00, 0x01, 0x01, 0x00, 0x00};
    write_i16_le(&enabled_with_torque[0], MAX_TORQUE);
    auto val = ecu.poll(current_time_ms).value();
...
void test_switch_debounce() {
    ...
    current_time_ms += 20;
    std::optional<CAN_message_t> msg = ecu.poll(current_time_ms);
    ...
    /* Wait 500ms (halfway through the debounce period). Make sure we keep sending
     * pedal positions so we don't time out as well. */
    wait_while_sending_pedal_positions(&ecu, &current_time_ms, 500, THROTTLE1_LOW, THROTTLE2_LOW, BRAKE_PRESSURE_MIN);
    msg = ecu.poll(current_time_ms);
    TEST_ASSERT(msg != std::nullopt);
